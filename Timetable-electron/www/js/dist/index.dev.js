...
var main = require('electron').remote.require('./main'); //acess export functions in main


var _require$remote = require('electron').remote,
    dialog = _require$remote.dialog,
    Menu = _require$remote.Menu,
    MenuItem = _require$remote.MenuItem,
    shell = _require$remote.shell,
    systemPreferences = _require$remote.systemPreferences,
    nativeTheme = _require$remote.nativeTheme;
...
function textboxmenu() {
  var text_box_menu = new Menu.buildFromTemplate([{
    role: 'cut'
  }, {
    role: 'copy'
  }, {
  ...
  document.getElementById('name_put').addEventListener('contextmenu', function (event) {
    popupmenu(event);
  }, false);
  document.getElementById('wallpaper_pathrepresenter').addEventListener('contextmenu', function (event) {
    popupmenu(event);
  }, false);
  document.getElementById('pathrepresenter').addEventListener('contextmenu', function (event) {
    popupmenu(event);
  }, false);
  document.getElementById('detail_put').addEventListener('contextmenu', function (event) {
    popupmenu(event);
  }, false);
...
var config = {
  data: {
    key: "TT01",
    theme: "device",
    //sets theme, defaults to time based
    backgroundimg: 'default',
    hilight_engine: false,
    //hilight engine whether to run or not
    colorpallet: 'default',
    animation: true,
    tiles: true,
    empty_rows: false,
  ...
  properties: {
    ...
    max: 0,
    min: 24,
    //Swapped because big brain, big big brain
    overwrite: null,
    ...
    startup: true,
    colors_changed: true,
    //re-render color pannel when this is true
    clocking: false,
    // is clock ticking
    management: false,
    quimk_start: -1,
  ...
  save: function save() {
    var ToFileSystem, ToStorageAPI;
    return regeneratorRuntime.async(function save$(_context3) {
      while (1) {
        switch (_context3.prev = _context3.next) {
          case 0:
            ToStorageAPI = function _ref2() {
              return regeneratorRuntime.async(function ToStorageAPI$(_context2) {
                while (1) {
            ...
            ToFileSystem = function _ref() {
              return regeneratorRuntime.async(function ToFileSystem$(_context) {
                while (1) {
  ...
  load: function load() {
    ...
    if (config.baseconfig.use_alt_storage == true) {
      ...
      if (fs.existsSync(config.baseconfig.alt_location.toString() + "/TT001_cfg config.json")) {
        ...
        //file does not exist, was moved, deleted or is inaccesible
        config.data = JSON.parse(localStorage.getItem("TT001_cfg"));
  ...
  backup: function backup() {
    var always_on_top_state, date, filepath;
    return regeneratorRuntime.async(function backup$(_context4) {
      while (1) {
  ...
  restore: function restore() {
    var always_on_top_state;
    return regeneratorRuntime.async(function restore$(_context5) {
      while (1) {
              ...
              if (filepath.canceled == true) {//diologue ccanceled
                ...
                fs.readFile(filepath.filePaths[0], 'utf-8', function (err, data) {
                  ...
                  if (err) {
                    ...
                    if (fileout.key == "TT01") {
                      //check if this file is a timetable backup file
                      config.data = fileout;
                      config.save();
                      maininitalizer();
  ...
  selectlocation: function selectlocation() {
    var always_on_top_state, path;
    return regeneratorRuntime.async(function selectlocation$(_context6) {
      while (1) {
          ...
          case 0:
            ...
            if (config.baseconfig.alt_location != undefined) {
              path = dialog.showOpenDialog({
                ...
                defaultPath: config.baseconfig.alt_location.toString()
              });
            } else {
              path = dialog.showOpenDialog({
                properties: ['createDirectory', 'openDirectory'],
...
var table = {
  data_render: function data_render() {
    ...
    if (config.data.table1_db[i] == null || undefined) {
      ...
      for (i = 0; i < config.data.table1_db.length; i++) {
        ...
        if (config.data.table1_db[i].deleted != true && config.data.table1_db[i].show == config.data.table_selected) {
          var starthraw = Number(config.data.table1_db[i].start) - config.data.table1_db[i].start % 1; // taking away the modulus 1 of itself removes the remainder
    ...
    function build_block_db1(index) {
      ...
      /* + '<br>' + starthr+':'+startminute+' '+startmeridian+' - '+endhr+':'+endminute+' '+endmeridian*/
      ; //info doots

      var doot = document.createElement('div');
      doot.setAttribute('class', 'infodoot');

      if (config.data.table1_db[index].start < config.properties.min + 3) {
        //Set the doot to flip up or down depending on the pannels position
        doot.style.top = '0vh';
      ...
      name_tab_content.innerHTML = config.data.table1_db[index].name; //name_tab_content.setAttribute("colspan", 2);
      ...
      time_tab.classList = "nowrap"; //time_tab.setAttribute("colspan", 2);
      ...
      if (config.data.table1_db[index].detail != "" && config.data.table1_db[index].detail != undefined) {
        ...
        var detail_content = document.createElement("td"); //detail_content.setAttribute("colspan", 2);
    ...
    function validate() {
      ...
      if (config.data.empty_rows == false) {
        ...
        if (config.data.table1_db.length < 3) {
          //normalization makes life easier fror small table users
  ...
  hilight_engine_go_vroom: function hilight_engine_go_vroom() {
    if (config.data.hilight_engine) {
      ...
      while (query[_i3] != null || query[_i3] != undefined) {
        query[_i3].addEventListener('mouseover', function () {
  ...
  quick_add: function quick_add() {
    var quick_add_menu, quimk_popup;
    return regeneratorRuntime.async(function quick_add$(_context7) {
      while (1) {
        switch (_context7.prev = _context7.next) {
          case 0:
            quimk_popup = function _ref3(e) {
              //Popup the menu
              e.preventDefault();
              e.stopPropagation();
              quick_add_menu.popup({
                window: require('electron').remote.getCurrentWindow()
              });
            };

            //quick add context menus
            //menu
            quick_add_menu = new Menu.buildFromTemplate([{
              label: 'Add item here',
              click: function click() {
                //Clicks to add new fills time in edit pannel
                UI.manage_toggle();
                manage.dialogue.open();
...
var manage = {
  ...
  render_tables: function render_tables() {
    ...
    var table0_button = document.createElement('div');
    table0_button.setAttribute("class", "table_bar");
    var titlespan0 = document.createElement('span');
    ...
    var new_table_button = document.createElement('div');
    ...
    function renderbar(index) {
      ...
      var confirmimg = document.createElement('div');
      ...
      var tab_put = document.createElement('input');
      tab_put.setAttribute("class", "tab_put"); //inject into document
      ...
      document.getElementById('tablespace_render').appendChild(table_bar); //make fucntion
    ...
    function build_bar_db1(index) {
      ...
      while (config.data.table_details[i] != null) {
        if (config.data.table_details[i].identifier == config.data.table1_db[index].show && config.data.table_details[i].deleted != true) {
          //compare this instence to indentifiers of other tables
          homeless = false;
        }
      ...
      tempblock.style.boxShadow = "0vh 0vh 0.5vh 0vh hsl(" + config.data.table1_db[index].color.hue + "," + config.data.table1_db[index].color.sat + "%," + config.data.table1_db[index].color.light + "%)"; //build menu

      var sub_optionbar = document.createElement('div');
      ...
      tempblock.appendChild(sub_optionbar); //time processing
      ...
      if (config.data.table1_db[index].deleted) {
        ...
        tempblock.style.border = "0.3vh solid red"; //alow editing function
        ...
        var day_tab_row = document.createElement("tr"); //let day_tab_head = document.createElement("td");
        ...
        day_tab_content.setAttribute("colspan", 2); //day_tab_row.appendChild(day_tab_head);
        ...
        time_tab_row.appendChild(time_tab);
        sub_tab.appendChild(time_tab_row);
        tempblock.appendChild(sub_tab); //allow editing function
      ...
      if (config.data.table1_db[index].show == 0) {
        // this dataset is homeless
        ...
        noot.style.color = 'red';
          ...
          while (config.data.table_details[_i4] != null) {
            if (config.data.table_details[_i4].identifier == Number(config.data.table1_db[index].show) && config.data.table_details[_i4].deleted != true) {
              noot.innerHTML = config.data.table_details[_i4].purpose;
              break;
            }
  ...
  dialogue: {
    ...
    open: function open() {
      ...
      if (config.properties.colors_changed == true) {
    ...
    save: function save() {
      ...
      var entryisvalid = true; //get day select, no validation, because default is valid

      tempentry.day = Number(document.getElementById('day_put').value); //get color select, no validation, because default is valid
      ...
      config.data.previous_colors.push(tempentry.color);
      config.properties.colors_changed = true; //Course Code is not required and can be anything, even nothing

      tempentry.detail = document.getElementById('detail_put').value; //Get Name feild
      ...
      var percentage_start = Number(start_time_raw.slice(0, 2) / 1
      /*I divide it by one becasue the scripting engine is drunk*/
      + start_time_raw.slice(3) / 60);
      var percentage_end = Number(end_time_raw.slice(0, 2) / 1
      /*I divide it by one becasue the scripting engine is drunk*/
      + end_time_raw.slice(3) / 60);
    ...
    saveplus: function saveplus() {
      ...
      if (entryisvalid) {
        notify["new"]('Confirmation', document.getElementById('name_put').value + ' was saved, U may now add another'); //no clear function needed, the clearfeild action btns will fufill this task
...
var UI = {
  initalize: function initalize() {
      ...
      setTimeout(function () {
        ...
        while (config.data.table_details[i] != null) {
          ...
          if (config.data.table_details[i].deleted != true && config.data.table_details[i].identifier == vewalue) {
            //check view put value against saved table details
            document.getElementById('view_put_text').innerHTML = config.data.table_details[i].purpose;
            break; //found it
          }
    ...
    document.getElementById('erraser').addEventListener('click', manage.dialogue.clear); //Initalize day_put selector
    ...
    document.getElementById('Manage_button_btn').addEventListener('click', UI.manage_toggle); //Switch triggers
    ...
    document.getElementById('select_btn').addEventListener('click', function () {
      //Select the configuration location
      config.selectlocation();
      config.properties.changed = true;
    });
  ...
  hue_selec: function hue_selec(hue) {
    config.data.colorpallet = hue;
    config.save();
    UI.setting.set_theme();
  },
  toggle_alwaysontop: function toggle_alwaysontop() {
  ...
  manage_toggle: function manage_toggle() {
    UI.close_tile();
  ...
  setting: {
    ...
    animation: {
      ...
      setpostition: function setpostition() {
        switch (process.platform) {
          ...
          default:
            //Mac OS && windows
            if (systemPreferences.getAnimationSettings().shouldRenderRichAnimation == true) {
              //animations preffered by system only works on windows and wackOS
              if (config.data.animation) {
                document.getElementById('Animations_switch_container').className = 'switch_container_active';
                document.getElementById('nomation').href = "";
              } else {
    ...
    wallpaper: {
      set_wallpaper: function set_wallpaper() {
        var useCSS, useUserSelected, useDesktop;
        return regeneratorRuntime.async(function set_wallpaper$(_context9) {
          while (1) {
            switch (_context9.prev = _context9.next) {
              case 0:
                useDesktop = function _ref6() {
                  return regeneratorRuntime.async(function useDesktop$(_context8) {
                    while (1) {
                      switch (_context8.prev = _context8.next) {
                        case 0:
                          wallpaper.get().then(function (wallpaperpath) {
                ...
                useUserSelected = function _ref5() {
                ...
                if (config.data.backgroundimg != null || undefined) {
                  ...
                  //no wallperper, clear and use css wallpaper
                  useCSS();
                }
      ...
      select_wallpaper: function select_wallpaper() {
        return regeneratorRuntime.async(function select_wallpaper$(_context10) {
          while (1) {
            switch (_context10.prev = _context10.next) {
              case 0:
                //User selects wallpaper
                dialog.showOpenDialog({
                  //electron async file dialogue
...
var notify = {
  preset_height: 22,
  //2 more than the height in the css
  previous_type: 1,
  animate_old: true,
  //turn on and off old notification Animation
  current: 0,
  //Current is incimented every time theres a new notifyer
  resizecheck: window.addEventListener('resize', function () {
    notify.clearall();
  }),
  "new": function _new(title, body, fx, bdytitle) {
    var _this2 = this;
    ...
    setTimeout(function () {
      tempnotif.style.transform = 'translate(0vw,0vh)'; //manuver old notifications out of the way

      if (_this2.animate_old) {
        if (document.getElementById('notif' + Number(_this2.current - 1))) {
          //stars at -1 because 1 less than the latest notification
          document.getElementById('notif' + Number(_this2.current - 1)).style.transform = 'translate(0vw,-' + _this2.preset_height + 'vh)';
        }

        if (document.getElementById('notif' + Number(_this2.current - 2))) {
          document.getElementById('notif' + Number(_this2.current - 2)).style.transform = 'translate(0vw,-' + _this2.preset_height * 2 + 'vh)';
        }

        if (document.getElementById('notif' + Number(_this2.current - 3))) {
          document.getElementById('notif' + Number(_this2.current - 3)).style.transform = 'translate(0vw,-' + _this2.preset_height * 3 + 'vh)';
        }

        if (document.getElementById('notif' + Number(_this2.current - 4))) {
          document.getElementById('notif' + Number(_this2.current - 4)).style.transform = 'translate(0vw,-' + _this2.preset_height * 4 + 'vh)';
        }

        if (document.getElementById('notif' + Number(_this2.current - 5))) {
          document.getElementById('notif' + Number(_this2.current - 5)).style.transform = 'translate(0vw,-' + _this2.preset_height * 5 + 'vh)';
        }

        if (document.getElementById('notif' + Number(_this2.current - 6))) {
          document.getElementById('notif' + Number(_this2.current - 6)).style.transform = 'translate(0vw,-' + _this2.preset_height * 6 + 'vh)';
        }

        if (document.getElementById('notif' + Number(_this2.current - 7))) {
          document.getElementById('notif' + Number(_this2.current - 7)).style.transform = 'translate(0vw,-' + _this2.preset_height * 7 + 'vh)';
        }
      }
    }, 50); //Slide into view
    ...
    if (typeof fx == 'function') {
      //There is a function, use X button
      ...
      xbutton.addEventListener('click', function () {
        event.stopImmediatePropagation(); //close app

        setTimeout(function () {
          tempnotif.style.opacity = '0.0';
        }, 100); //yee.style.zIndex = '-999';
    ...
    tempnotif.addEventListener('click', function () {
      ...
      //close regardless of function
      setTimeout(function () {
        _this3.style.opacity = '0.0';
        _this3.style.zIndex = '-999';
      }, 100);
      this.style.transform = 'translate(35vw,0)';
    });
  },
  clearall: function clearall() {
    if (document.getElementById('notif' + Number(this.current))) {
      //nep them from latest going up
...
var utility = {
  ...
  clipboard: function clipboard(textpush) {
  ...
  rand: {
    ...
    number: function number(max, min) {
